...
class PatternLayout extends Layout {
  ...
  String format(LoggingEvent event) {
    ...
    for (var m in specifier.allMatches(pattern)) {
      ...
      switch(type) {
        case 's': {
          replacement = _padAndTruncate(event.level.toString(), flags, width, precision);
          break;
        }
        case 'm': {
          replacement = _padAndTruncate(event.message, flags, width, precision);
          break;
        }
        case 'l': {
          replacement = _padAndTruncate(event.loggerName, flags, width, precision);
          break;
        }
        case 'd': {
          ...
          replacement = _padAndTruncate(dateFormat.format(event.instant), flags, width, precision);
          break;
        }
        case 'n': {
  ...
  String _padAndTruncate(String input, String? flags, String? width, String? precision) {
    String result = input;
    bool rightPad = false;
